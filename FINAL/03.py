...
# -*- coding: utf-8 -*-
"""
03_map_lean_cases.py
--------------------
Build a φ₂–H₂O map for *every* rich-burn case found in
    • cases_setup.json      (global data & mass-flow splits)
    • rich_cases.json       (rich flame results)

Run after 01_setup_cases.py and 02_run_rich_stage.py.
---------------------------------------------------------------------------
"""
...
# ───────────────────────────────────────────────────────────────────────────────
# USER CONSTANTS (edit freely)
# ───────────────────────────────────────────────────────────────────────────────
T_H2O       = 353.15          # K  (80 °C liquid / vapour injection)
GRID_NODES  = 10              # φ₂ × H₂O resolution per map
...
# ───────────────────────────────────────────────────────────────────────────────
# 1.  GENERIC LEAN-STAGE SOLVER (unchanged)
# ───────────────────────────────────────────────────────────────────────────────
def run_case(args: Tuple[float, float, dict,
    ...
    # ----- data from rich burn -------------------------------------------------
    P_mix = data["P_inlet_Pa"]          # ≈ mixer pressure (rich flame outlet)
    X_exh = data["X_exh"]


    # Build the gas once so we know the species order
    g_exh = ct.Solution(MECH)
    g_exh.X = X_exh
    g_exh.HP = h_exh, P_mix

    # >>> NEW: convert list → dict for mixture_properties <<<
    if isinstance(X_exh, list):
        spec_names = g_exh.species_names                # same order
        X_exh_dict = {s: X_exh[i] for i, s in enumerate(spec_names)
                      if X_exh[i] > 0.0}                # drop exact zeros
    else:
        X_exh_dict = X_exh

    # ----- split available air -------------------------------------------------
    AVALIABLE_AIR = TOTAL_AIR * (1.0 - 0.0525)        # reserve 5.25 % for film cool
    ...
    # convert to total-T at assumed M=0.3
    ...
    return (gas.equivalence_ratio(), mdot_H2O,
            float(flame.T.max()),
            float((X_NO + X_N2O) * 1e6),
            float(flame.X[idx("O2"),  -1]),
            float(flame.X[idx("H2O"), -1]),
            float(gamma), Hdot_in, Hdot_out, Qdot_rel,
            float(gas_TIT.T), float(mdot_TIT))

# ───────────────────────────────────────────────────────────────────────────────
# helper: fractional-air→φ₂ relationship (unchanged)
# ───────────────────────────────────────────────────────────────────────────────
...
def map_single_case(setup: dict, rich: dict, mech: str) -> None:
    ...
    T_in      = setup["T_inlet_K"]            # K  (for lean-air enthalpy)
    P_mix     = rich ["P_inlet_Pa"]           # Pa (≈ mixer pressure)

    # lean-stage inlet air enthalpy
    g_air = ct.Solution(mech); g_air.TPX = T_in, P_mix, X_AIR
    h_air2 = g_air.enthalpy_mass             # J kg⁻¹

    # pre-compute water enthalpies (vapour vs. liq.)
    ...
    F_SWEEP = f_for_phi2(phi2_targets,
                         ...
                         TOTAL_AIR=TOTAL_AIR,
                         mdot_air1=mdot_air1)
    W_SWEEP = np.linspace(H2O_MIN, H2O_MAX, GRID_NODES)
...
def main() -> None:
    ...
    # iterate over all matching power points
    produced = []
    ...
    # write a tiny text index for convenience
    if produced:
