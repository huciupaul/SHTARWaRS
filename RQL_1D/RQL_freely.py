...
width_lean = 0.02    # m – domain length for free lean flame
...
print(f"X_NO  (burnt)         : {X_NO_out*1e6:10.2f} ppm")
print(f"X_N₂O (burnt)         : {X_N2O_out*1e6:10.2f} ppm")
print("-----------------------------------------------------------\n")
...
X_mix1, mdot_mix1, _ = mixture_properties(
    X1=X_exh, X2=X_AIR,
    mdot1=mdot_H2+mdot_air1, mdot2=mdot_air2
)
...
h_mix = (
    gas_exh.enthalpy_mass +
    (mdot_air2/mdot_mix1) * gas_air2.enthalpy_mass +
    (MDOT_H2O/mdot_mix1)  * gas_H2O.enthalpy_mass
)

mixed_gas = ct.Solution(MECH)
mixed_gas.set_equivalence_ratio(
    phi=gas_exh.equivalence_ratio(), fuel=X_H2, oxidizer=X_AIR
)
...
flame_lean.set_refine_criteria(ratio=6.0, slope=0.02, curve=0.04)
