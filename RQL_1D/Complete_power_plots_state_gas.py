...
def run_case(args: Tuple[float, float, dict,
    ...
    # -----------------------------------------------------------------------------
    # NEW: turbine-inlet energy flux via mdot * Cp * T -----------------------------
    M_TIT = 0.30  # assumed turbine-inlet Mach number
    cp_TIT = gas_TIT.cp_mass  # J kg-1 K-1   (variable, species-weighted)
    gamma_TIT = cp_TIT / gas_TIT.cv_mass  # γ(TIT) for total-T conversion

    # total (stagnation) temperature at M = 0.30   ⟹  T₀ = T·[1+(γ−1)/2·M²]
    T_total_TIT = gas_TIT.T * (1.0 + 0.5 * (gamma_TIT - 1.0) * M_TIT ** 2)  # K
    # (Equation from the steady-flow energy equation for a perfect gas :contentReference[oaicite:0]{index=0})

    # turbine-inlet enthalpy flux (≈ chemical heat available to the turbine)
    Hdot_out = mdot_TIT * cp_TIT * T_total_TIT  # W

    # -----------------------------------------------------------------------------
    # keep everything else identical to your original block ------------------------
    gamma = gamma_TIT  # you already need γ later
    ...
    return (gas.equivalence_ratio(),
            ...
            float(gamma),
            Hdot_in,  # sensible IN    (unchanged)
            Hdot_out,  # NEW: Cp·T total energy flux
            Qdot_release,  # chem. heat in second stage (unchanged)
            float(gas_TIT.T),  # TIT *static* for logging
            float(mdot_TIT))
